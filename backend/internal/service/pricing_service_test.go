...
func TestBillingServiceGPT6AstraUsesOfficialPricingAcrossTiersAndLongContext(t *testing.T) {
	...
	require.InDelta(t, usdToCNY(100_000*10e-6), boundary.InputCost, 1e-12)
	require.InDelta(t, usdToCNY(100_000*12.5e-6), boundary.CacheCreationCost, 1e-12)
	require.InDelta(t, usdToCNY(72_000*1e-6), boundary.CacheReadCost, 1e-12)
	require.InDelta(t, usdToCNY(10*50e-6), boundary.OutputCost, 1e-12)
	...
	for _, tier := range tiers {
		t.Run(tier.name, func(t *testing.T) {
			...
			require.InDelta(t, usdToCNY(100_000*10e-6*tier.priceScale*2), cost.InputCost, 1e-12)
			require.InDelta(t, usdToCNY(100_000*12.5e-6*tier.priceScale*2), cost.CacheCreationCost, 1e-12)
			require.InDelta(t, usdToCNY(73_000*1e-6*tier.priceScale*2), cost.CacheReadCost, 1e-12)
			require.InDelta(t, usdToCNY(10*50e-6*tier.priceScale*1.5), cost.OutputCost, 1e-12)
		})
	}
}
...
func TestGPT6AstraDedicatedFallbacksUseOfficialRates(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.InDelta(t, usdToCNY(20e-6), pricing.InputPricePerTokenPriority, 1e-12)
			require.InDelta(t, usdToCNY(50e-6), pricing.OutputPricePerToken, 1e-12)
			require.InDelta(t, usdToCNY(100e-6), pricing.OutputPricePerTokenPriority, 1e-12)
			require.InDelta(t, usdToCNY(12.5e-6), pricing.CacheCreationPricePerToken, 1e-12)
			require.InDelta(t, usdToCNY(25e-6), pricing.CacheCreationPricePerTokenPriority, 1e-12)
			require.InDelta(t, usdToCNY(1e-6), pricing.CacheReadPricePerToken, 1e-12)
			require.InDelta(t, usdToCNY(2e-6), pricing.CacheReadPricePerTokenPriority, 1e-12)
