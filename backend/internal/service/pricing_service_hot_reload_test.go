...
// fallback 是配置来源，不得因启动时的目录缓存继续复活已撤销的价卡。
func TestPricingHotReload_FallbackOnlyUsesCurrentSourceAfterRestart(t *testing.T) {
	dir := t.TempDir()
	cfg := &config.Config{}
	cfg.Pricing.DataDir = dir
	cfg.Pricing.FallbackFile = filepath.Join(dir, "fallback.json")
	original := `{` + hotReloadModelJSON("custom-a", 1e-6, 2e-6) + `,` + hotReloadModelJSON("custom-b", 2e-6, 4e-6) + `}`
	require.NoError(t, os.WriteFile(cfg.Pricing.FallbackFile, []byte(original), 0600))
	svc := NewPricingService(cfg, nil)
	require.NoError(t, svc.Initialize())
	t.Cleanup(svc.Stop)
	require.Len(t, svc.pricingData, 2)
	current := `{` + hotReloadModelJSON("custom-b", 3e-6, 6e-6) + `}`
	require.NoError(t, os.WriteFile(cfg.Pricing.FallbackFile, []byte(current), 0600))
	require.NoError(t, svc.reloadCustomPricingLayers())
	require.NotContains(t, svc.pricingData, "custom-a")
	require.InDelta(t, 3e-6, svc.pricingData["custom-b"].InputCostPerToken, 1e-12)
	restarted := NewPricingService(cfg, nil)
	require.NoError(t, restarted.Initialize())
	t.Cleanup(restarted.Stop)
	require.NotContains(t, restarted.pricingData, "custom-a")
	require.InDelta(t, 3e-6, restarted.pricingData["custom-b"].InputCostPerToken, 1e-12)
}
