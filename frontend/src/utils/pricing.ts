...
export function resolveIntervalPrices(iv: UserPricingInterval, base: TokenPrices): UserPricingInterval {
  ...
  return {
    ...
    // 仅在价卡声明 1h 项时继承一般写入覆盖；计算回退不新增展示项。
    cache_write_1h_price: iv.cache_write_1h_price ?? (base.cache_write_1h_price == null
      ? null
      : iv.cache_write_price ?? price(null, iv.cache_write_multiplier, base.cache_write_1h_price)),
