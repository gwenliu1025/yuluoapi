...
describe('resolveIntervalPrices', () => {
  it.each([
    { cache_write_1h_price: undefined, cache_write_price: null },
    { cache_write_1h_price: null, cache_write_price: null },
    { cache_write_1h_price: null, cache_write_price: 20 },
    { cache_write_1h_price: null, cache_write_price: 0 },
    { cache_write_1h_price: null, cache_write_price: null, cache_write_multiplier: 2 }
  ])('未声明 1h 价时不新增独立展示项 %j', (fields) => {
    for (const missing of [undefined, null]) {
      const actual = resolveIntervalPrices({ ...interval, ...fields, explicit_cache_read_price: 3 }, { ...base, cache_write_1h_price: missing })
      expect(actual.cache_write_1h_price).toBeNull()
      expect(actual.explicit_cache_read_price).toBe(3)
    }
  })
  it('基础卡未声明 1h 时仍保留区间显式零价', () => {
    const actual = resolveIntervalPrices({ ...interval, cache_write_1h_price: 0 }, { ...base, cache_write_1h_price: null })
    expect(actual.cache_write_1h_price).toBe(0)
  })
